...
### Command Line Options
  ...
  | -restore_from_disk "name_of_capture" | restore windows from (named) capture on hard drive and exit. Note that a second restore using the same command line is needed to restore new window launched during the first restore.
